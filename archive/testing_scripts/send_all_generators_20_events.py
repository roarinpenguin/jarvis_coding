...
# List of all generators by category
generators = {
    ...
    "identity_access": [
        ...
        "beyondtrust_privilegemgmtwindows",
        "cisco_duo",
        "cyberark_conjur",
        "cyberark_pas",
        "hashicorp_vault",
...
def send_events_for_generator(product: str, count: int = 20) -> bool:
    ...
    try:
        ...
        print(f"   ⏰ {product}: Timeout after 30 seconds")
        return False
...
def main():
    """Send 20 events for all generators"""
    print("🚀 SENDING 20 EVENTS FOR ALL GENERATORS")
    print("=" * 60)
    ...
    for category, gen_list in generators.items():
        ...
        for generator in gen_list:
            if send_events_for_generator(generator, 20):
                successful += 1
            else:
                failed += 1
